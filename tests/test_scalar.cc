...
{
    using float_length_t = dim::scalar<float, dim::mechanical_dimension<1, 0, 0>>;
    using double_length_t = dim::scalar<double, dim::mechanical_dimension<1, 0, 0>>;
...
{
    using number_t = dim::scalar<double, dim::mechanical_dimension<1, 0, 0>>;
...
{
    using number_t = dim::scalar<double, dim::mechanical_dimension<0, 0, 0>>;
    using length_t = dim::scalar<double, dim::mechanical_dimension<1, 0, 0>>;
...
TEST_CASE("scalar: exposes dimension")
{
    using number_t = dim::scalar<double, dim::mechanical_dimension<0, 0, 0>>;
    using length_t = dim::scalar<double, dim::mechanical_dimension<1, 0, 0>>;
    CHECK((std::is_same<number_t::dimension, dim::mechanical_dimension<0, 0, 0>>::value));
    CHECK((std::is_same<length_t::dimension, dim::mechanical_dimension<1, 0, 0>>::value));
}
...
{
    using length_t = dim::scalar<double, dim::mechanical_dimension<1, 0, 0>>;
    using wavenumber_t = dim::scalar<double, dim::mechanical_dimension<-1, 0, 0>>;
...
TEST_CASE("scalar: provides abs function")
{
    ...
    length_t const x = dim::abs(length_t{12});
    length_t const y = dim::abs(length_t{-34});
    CHECK(x == length_t{12});
    CHECK(y == length_t{34});
}
...
TEST_CASE("scalar: provides hypot function")
{
    ...
    length_t const x = dim::hypot(length_t{3}, length_t{4});
    CHECK(x == length_t{5});
}
...
TEST_CASE("scalar: provides pow function")
{
    ...
    using volume_t = dim::scalar<double, dim::mechanical_dimension<3, 0, 0>>;
    using density_t = dim::scalar<double, dim::mechanical_dimension<-3, 0, 0>>;

    length_t const x{4};
    CHECK(dim::pow<2>(x) == area_t{16});
    CHECK(dim::pow<3>(x) == volume_t{64});
    CHECK(dim::pow<-3>(x) == density_t{1.0 / 64});
}
...
TEST_CASE("scalar: example: mechanical quantities")
{
    ...
    using momentum_t = dim::scalar<double, dim::mech::momentum>;
    ...
    double const some_ratio = dim::sqrt(mass * energy) / momentum;
    (void) some_ratio;
}
