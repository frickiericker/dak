...
{
    using vec1 = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 1>;
    using vec2 = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 2>;
    using vec3 = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using vec4 = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 4>;
    CHECK(sizeof(vec1) > 0);
    CHECK(sizeof(vec2) > 0);
    CHECK(sizeof(vec3) > 0);
    CHECK(sizeof(vec4) > 0);
...
{
    using float_displace_t = dim::vector<float, dim::mechanical_dimension<1, 0, 0>, 3>;
    using double_displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
...
{
    using number_t = dim::vector<double, dim::mechanical_dimension<0, 0, 0>, 3>;
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
...
TEST_CASE("vector: disallows dimension-altering conversion")
{
    using number_t = dim::vector<double, dim::mechanical_dimension<0, 0, 0>, 3>;
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using velocity_t = dim::vector<double, dim::mechanical_dimension<1, 0, -1>, 3>;
...
{
    using float_displace_t = dim::vector<float, dim::mechanical_dimension<1, 0, -1>, 3>;
    using double_displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, -1>, 3>;
...
{
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, -1>, 3>;
    using length_t = dim::scalar<double, dim::mechanical_dimension<1, 0, -1>>;
...
TEST_CASE("vector: exposes spatial dimension")
{
    ...
    using vec3 = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using vec4 = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 4>;
    using vec9 = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 9>;
...
{
    using speed_t = dim::scalar<double, dim::mechanical_dimension<1, 0, -1>>;
    using velocity_t = dim::vector<double, dim::mechanical_dimension<1, 0, -1>, 3>;
...
{
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using frequency_t = dim::scalar<double, dim::mechanical_dimension<0, 0, -1>>;
    using velocity_t = dim::vector<double, dim::mechanical_dimension<1, 0, -1>, 3>;
...
{
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using duration_t = dim::scalar<double, dim::mechanical_dimension<0, 0, 1>>;
    using velocity_t = dim::vector<double, dim::mechanical_dimension<1, 0, -1>, 3>;
...
{
    using force_t = dim::vector<double, dim::mechanical_dimension<1, 1, -2>, 3>;
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using energy_t = dim::scalar<double, dim::mechanical_dimension<2, 1, -2>>;

    force_t const x{1, 2, 3};
    displace_t const y{4, 5, 6};
    CHECK(dim::dot(x, y) == energy_t{4 + 10 + 18});
}

TEST_CASE("vector: provides squared_norm function")
{
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using area_t = dim::scalar<double, dim::mechanical_dimension<2, 0, 0>>;

    displace_t const x{1, 2, 3};
    CHECK(dim::squared_norm(x) == area_t{1 + 4 + 9});
}

TEST_CASE("vector: provides norm function")
{
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 2>;
    using length_t = dim::scalar<double, dim::mechanical_dimension<1, 0, 0>>;

    displace_t const x{3, 4};
    CHECK(dim::norm(x) == length_t{5});
}

TEST_CASE("vector: provides cross function for three-dimensional vectors")
{
    using displace_t = dim::vector<double, dim::mechanical_dimension<1, 0, 0>, 3>;
    using area_vector_t = dim::vector<double, dim::mechanical_dimension<2, 0, 0>, 3>;

    displace_t const x{1, 2, 3};
    displace_t const y{4, 5, 6};
    CHECK(dim::cross(x, y) == area_vector_t{2 * 6 - 3 * 5, 3 * 4 - 1 * 6, 1 * 5 - 2 * 4});

    // Invariant
    CHECK(dim::cross(x, x) == area_vector_t{0, 0, 0});
    CHECK(dim::cross(y, y) == area_vector_t{0, 0, 0});
}

TEST_CASE("vector: example: mechanical quantities")
{
    using mass_t = dim::scalar<double, dim::mech::mass>;
    using duration_t = dim::scalar<double, dim::mech::time>;
    using energy_t = dim::scalar<double, dim::mech::energy>;

    using displace_t = dim::vector<double, dim::mech::length, 3>;
    using velocity_t = dim::vector<double, dim::mech::speed, 3>;
    using momentum_t = dim::vector<double, dim::mech::momentum, 3>;
    using force_t = dim::vector<double, dim::mech::force, 3>;
    ...
    energy_t const energy = dim::dot(force, displacement);
    double const some_ratio = dim::sqrt(mass * energy) / dim::norm(momentum);
    (void) some_ratio;
}
